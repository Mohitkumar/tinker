...
class OTelBackend(ObservabilityBackend):
    ...
    def __init__(self, config: dict | None = None) -> None:
        cfg = config or {}
        self._os_url = (cfg.get("opensearch_url") or cfg.get("url") or "").rstrip("/")
        self._os_key = cfg.get("opensearch_api_key") or cfg.get("api_key")
        self._prom_url = (cfg.get("prometheus_url") or "").rstrip("/")
        self._log_index = cfg.get("log_index") or "otel-logs-*"
