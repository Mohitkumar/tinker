...
class TestCloudWatchMCPServer:
    ...
    async def test_handle_query_logs_returns_text_content(self, mock_backend_cls):
        ...
        mock_backend.query_logs = AsyncMock(
            return_value=[
                LogEntry(
        ...
        mock_backend._parse_since = MagicMock(
        ...
        result = await server._handle_query_logs(
            {"service": "payments", "query": "level:ERROR", "since": "1h"}
        )
    ...
    async def test_handle_detect_anomalies(self, mock_backend_cls):
        ...
        mock_backend = MagicMock()
        mock_backend.detect_anomalies = AsyncMock(
            return_value=[
                Anomaly(
                    service="payments",
                    metric="error_count",
